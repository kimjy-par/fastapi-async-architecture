...
async def test_read_by_id(database):
    ...
    user = await create_user(
        User(username="unittest_user", email="unit@test.com", is_activate=True)
    )
...
async def test_list_user_is_not_active(database):
    ...
    results = await user_repository.list_by_query(
        select(User).filter(User.is_activate == False)
    )
    users_from_db = results["results"]
...
async def test_create_user(database):
    ...
    user_schema = UserCreateRequest(
        username="create", email="create@test.com", is_activate=False
    )
...
async def test_update_user(database):
    user_repository = UserRepository(database.session)
    user = await create_user(
        User(username="update", email="update@test.com", is_activate=False)
    )

    schema_to_update = UserUpdateRequest(
        username="change", email="change@test.com", is_activate=True
    )
...
async def test_delete_user(database):
    user_repository = UserRepository(database.session)
    user = await create_user(
        User(username="delete", email="delete@test.com", is_activate=True)
    )
